...
class Runner:
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step > 100001:
                ...
                pa_loss = torch.mean(abs(p_unc - a_unc))
                loss = loss + pa_loss
    ...
    def sphere_tracing(self, ray_o, ray_d):
        ...
        while True:
            unfinished_mask_start = (
                unfinished_mask_start & (curr_sdf_start.abs() > sdf_threshold))
    ...
    def get_view_prior(self):
        ...
        for num in tqdm(range(0, psnumber)):
            ...
            pcount[num] = self.dataset.n_images-pr
    ...
    def priorNN(self):
        ...
        t_un_val = 1 * t_u_val
        t_c_train = t_c_train/(self.dataset.n_images + 2)
        t_c_val = t_c_val/(self.dataset.n_images + 2)
    ...
    def innerprior(self):
        ...
        surf = ax.scatter(punc[:, 0], punc[:, 1], punc[:, 2], c=pcount)
        ax.set_xlim(-1, 1)
        ax.set_ylim(-1, 1)
        ax.set_zlim(-1, 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--case', type=str, default='D6153')
